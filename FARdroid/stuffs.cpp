...
#pragma pack(pop)
  ...
  HANDLE hFile = hFile = CreateFile(sPath, GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
  if (hFile != INVALID_HANDLE_VALUE)
  {
    DWORD writed = 0;
    WriteFile(hFile, "BM", 2, &writed, nullptr);

    WriteFile(hFile, &bmpfile_header, sizeof(bmpfile_header), &writed, nullptr);
    WriteFile(hFile, &bmp_dib_v3_header_t, sizeof(bmp_dib_v3_header_t), &writed, nullptr);
    ...
    for (int y = h - 1; y >= 0; y--)
    {
      ...
      for (int x = 0; x < w; x++)
      {
        ...
        WriteFile(hFile, &val, 3, &writed, nullptr);
      }
    }
